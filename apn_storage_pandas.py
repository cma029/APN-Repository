...
def store_apn_pandas(
    ...
    # Build an APN object and compute all invariants to store as a new row in the Parquet database.
    ...
    try:
        ...
        print(f"Error parsing polynomial {polynomial_terms} => {parse_err}")
        return
    ...
    if not apn_object.invariants.get("is_apn", False):
        print(f"The APN is not APN (is_apn False). Will not be stored.")
        return
    ...
    alg_deg_val = apn_object.invariants.get("algebraic_degree", None)
    if alg_deg_val is not None:
        alg_deg_val = int(alg_deg_val)
...
def load_apn_objects_for_field_pandas(field_dimension: int) -> List[APN]:
    ...
    for index, row in df_loaded.iterrows():
        try:
            ...
            apn_obj = parser.parse_univariate_polynomial(polynomial_data, field_n_val, irr_poly_val)
            if not hasattr(apn_obj, "invariants"):
                apn_obj.invariants = {}

            # Odds, will unify if JSON is a dictionary.
            ...
            # Odws, will unify if JSON is a dictionary.
